--// ReplicatedStorage.SharedModules.ContentPacks.FriendshipLevel2023.InventorySubDB.Food (ModuleScript)

local v1 = require(game.ReplicatedStorage.ClientDB.Inventory.EntryHelper).food
local v2 = {}
local v3 = {
    ["name"] = "Age-Up Potion",
    ["image"] = "rbxassetid://13619902566",
    ["rarity"] = "rare",
    ["donatable"] = false,
    ["anims"] = {
        ["hold"] = "PotionHold",
        ["use"] = "PotionDrink"
    },
    ["cannot_feed_to_doll"] = true,
    ["cannot_feed_to_egg"] = true,
    ["cannot_feed_to_self"] = true,
    ["custom_tooltip_description"] = "Grants 30 tasks of experience to a pet\'s age.",
    ["message"] = "Feed this to a pet to instantly age it.",
    ["model_handle"] = "AgePotion",
    ["player_must_confirm"] = true,
    ["force_journal_visibility"] = true,
    ["tool"] = "GenericTool",
    ["uses"] = 1
}
v2.pet_age_potion = v3
return v1(v2)